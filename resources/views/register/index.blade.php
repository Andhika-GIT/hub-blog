        ...
        <h1 class="h3 mb-3 fw-normal text-center">Registration Form</h1>
            <form action="/register" method="POST">
              ...
              <div class="form-floating">
                {{-- @error untuk mengecheck apakah ada error validasi dari controller --}}
                <input type="text" name="name" class="form-control rounded-top @error('name') is-invalid @enderror " id="name" placeholder="enter your name" value="{{ old('nama') }}">
                <label for="name">Name</label>
                @error('name')
                <div class="invalid-feedback">
                  {{-- $message merupakan pesan error validasi (baca dokumentasi laravel) --}}
                  {{ $message }}
                </div>
                @enderror
              </div>
